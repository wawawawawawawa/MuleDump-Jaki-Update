...
function item(id) {
	...
	var title = it[0];
	if (~it[2] && it[1] != 10) title += ' (T' + it[2] + ')';
	if (it[6]) title += '\nFeed Power: ' + it[6]
	if (it[5]) title += '\nFame Bonus: ' + it[5] + ' %'
	if (it[8]) title += '\nSoulbound'
	return $r.attr('title', title).css('background-position', '-' + it[3] + 'px -' + it[4] + 'px')
}
window.item = item
...
/*	if (this.data && compare(d, this.data.query.results.Chars)) {
	...
	for (var i = 0; i < carr.length; i++) {
		...
		if (this.opt('chdesc')) {
			...
			var boost = $('<img class="boost hidden" src="lib/boost.png" style="float:left;width:16px;height:16px;">');
			...
			for ( x = 0; x < boosts.length; x++ ) {
				if ( boosts[x] !== "0" ) {
					...
					if ( c.XpBoosted !== "0" ) {
						...
						boostHtml += "XpBooster : " + length;
					}
					if ( c.LDTimer !== "0" ) {
						length = Math.floor(Number(c.LDTimer)/60);
						boostHtml += "Loot Drop : " + length + " minutes" + '\n';
					}
					if ( c.LTTimer !== "0" ) {
						length = Math.floor(Number(c.LTTimer)/60);
						boostHtml += "Loot Tier : " + length + " minutes";
					}
					break;
				}
			}
			...
			var chtext = $('<div>')
				...
				if ( boost.hasClass('hidden') === false ) {
					chtext.append(boost.attr('title', boostHtml))
				}
				
				chtext.append($('<div>').text(cl[0] + ' ' + c.Level + ', #' + c.id))
				chtext.append($('<div>').text(c.CurrentFame + ' Fame ' + c.Exp + ' XP'))
				// WAWAWA PART
