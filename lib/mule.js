...
Mule.prototype.parse = function(data) {
	...
	d.Account = d.Account || {}
	var $name = $('<div>').addClass('name').text(d.Account.Name + ' : ' + (d.Char.length != undefined ? d.Char.length + ' Chars' : ' 1 Char') || '(No Name) : ' + (d.Char.length != undefined ? d.Char.length + ' Chars' : ' 0 Chars'));
	addstar(this.dom, d);
	var self = this;
	...
	for (var i = 0; i < carr.length; i++) {
		...
		if (this.opt('chdesc')) {
			...
			if ( $c instanceof jQuery ) {
				...
				var boost = $('<img class="flex-container noFlexAutoWidth boost hidden" style="flex-flow:column"></img>');
					...
					if ( boosts[x] !== "0" ) {
						...
						'<div class="flex-container noFlexAutoWidth noFlexAutoAlign" style="color:#fff; flex-flow:column">' 
							+ (( c.XpBoosted !== "0" ) ? '<div class="flex-container noFlexAutoAlign" style="justify-content:flex-start">' 
								+ '<div class="petItem" style="background-position: -' + XPBoost[3] + 'px -' + XPBoost[4] + 'px; border:solid 1px #000; margin-right:8px; align-self:center"></div>' 
								...
								+ '<div class="flex-container noFlexAutoAlign" style="flex-direction:column">' 
									+ '<div style="font-weight:700; font-size:1.5em; align-self:flex-end">' + Math.floor(Number(c.XpTimer)/60) + ' minutes</div>' 
							+ '</div></div>' : '') 
								
							+ (( c.LDTimer !== "0" ) ? '<div class="flex-container noFlexAutoAlign" style="justify-content:flex-start">' 
								+ '<div class="petItem" style="background-position: -' + LDBoost[3] + 'px -' + LDBoost[4] + 'px; border:solid 1px #000; margin-right:8px; align-self:center"></div>' 
								+ '<div class="flex-container noFlexAutoWidth pr5" style="flex-flow:column; flex-shrink:0; margin-right:8px">' 
									+ '<div style="font-weight:700; font-size:1.5em; align-self:flex-start">Loot Drop : </div>' 
								+ '</div>'
								+ '<div class="flex-container noFlexAutoAlign" style="flex-direction:column">' 
									+ '<div style="font-weight:700; font-size:1.5em; align-self:flex-end">' + Math.floor(Number(c.LDTimer)/60) + ' minutes</div>' 
							+ '</div></div>' : '') 
								
							+ (( c.LTTimer !== "0" ) ? '<div class="flex-container noFlexAutoAlign" style="justify-content:flex-start">' 
								+ '<div class="petItem" style="background-position: -' + LTBoost[3] + 'px -' + LTBoost[4] + 'px; border:solid 1px #000; margin-right:8px; align-self:center"></div>' 
								+ '<div class="flex-container noFlexAutoWidth pr5" style="flex-flow:column; flex-shrink:0; margin-right:8px">' 
									+ '<div style="font-weight:700; font-size:1.5em; align-self:flex-start">Loot Tier : </div>' 
								+ '</div>'
								+ '<div class="flex-container noFlexAutoAlign" style="flex-direction:column">' 
									+ '<div style="font-weight:700; font-size:1.5em; align-self:flex-end">' + Math.floor(Number(c.LTTimer)/60) + ' minutes</div>' 
							+ '</div></div>' : '') 
						+ '</div>');
					}
				}
			...
			'<div class="flex-container noFlexAutoWidth noFlexAutoAlign" style="color:#fff; flex-flow:column">' 
				+ '<div class="flex-container noFlexAutoAlign mr5">' + '<div class="flex-container noFlexAutoWidth" style="border:solid 1px #000; margin-right:8px; align-self:center; background-color: #545454">' 
	...
	CharNumData.push('<div class="flex-container noFlexAutoAlign noFlexAutoJustify noFlexAutoWidth" style="flex-flow:column; font-size:1.3em; font-weight:700"><div class="flex-container noFlexAutoAlign noFlexAutoJustify noFlexAutoWidth" style="flex-flow:row">');
	for (var i in ClassList) {
		CharNumData.push(( ClassList[i]['Num'] > 0 ) ? '<div class="flex-container" style="flex-flow:column"><div class="ClassType" style="background-position: ' + ClassList[i]['X'] + ' ' + ClassList[i]['Y'] + '"></div><div class="mr5 mt5">' + ClassList[i]['Num'] + '</div><div class="mr5 mt5">' + i + '</div></div>' : '' )
	}
	CharNumData.push('</div><div class="flex-container">\xa0</div><div class="flex-container">Total Base Fame : ' + NumberFormat(BaseFameTotal, ',') + ' Fame</div><div class="flex-container" style="flex-flow:column">Total Dead Fame : ' + NumberFormat(DeadFameTotal, ',') + ' Fame</div></div>');
