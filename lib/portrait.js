...
// css-compatible
    ...
    window.portrait = function (img, type, skin, tex1, tex2) {
        ...
        if ( !skinData ) return;
        var c = pcache[pCacheId(img, tex1, tex2)];
                ...
                <div class="flex-container noFlexAutoWidth" style="color: #fff; flex-flow: column;"> \
                    ' + ( (textures[tex1] ) ?
                        '<div class="flex-container" style="justify-content: flex-start;">\
                            ...
                            <div style="font-weight:700; font-size:17px; align-self:center">' + ("&nbsp;" + textures[tex1][0] || 'Default') + '</div>\
                        </div>' : ''
                    ) + ' \
                    ' + ( (textures[tex2] ) ?
                        '<div class="flex-container" style="justify-content: flex-start;">\
                            <div class="item noselect mr5" style="background-position: -' + small[3] + 'px -' + small[4] + 'px; border: solid 1px #000;">&nbsp;</div>\
                            <div style="font-weight:700; font-size:17px; align-self:center">' + ("&nbsp;" + textures[tex2][2] || 'Default') + '</div>\
                        </div>' : ''
                    ) + '\
                </div>\
            ');
